...
**Source:** Period data seeded from `time_periods.csv` (Wikidata extraction) and `periodo-dataset.csv` (PeriodO authority).
...
**Source Files:**
- `periodo-dataset.csv`: PeriodO data
- `period_lcsh_mapping_phase1_*.json`: LCSH mapping tables
...
**Source:** ONTOLOGY_PRINCIPLES.md (2025-12-26) â€” foundational architectural decision
...
|------------------|-------------------|--------------------|-----------------|
| **LOCATED_IN** | P131 (located in) | P7 (took place at) | â€” |
...
| **CAUSED** | P828 (has cause) | P15 (was influenced by) | â€” |
| **APPOINTED** | P39 (position held) | P14_1 (in the role of) | E13_Attribute_Assignment |
| **WITHIN_TIMESPAN** | â€” | P4 (has time-span) | â€” |
...
# **Appendix M: Identifier Safety Reference** ðŸ”´ **CRITICAL DEVELOPER REFERENCE**

## **M.1 Quick Decision Table: Can LLMs Process This?**

| What You Have | Example | LLM Safe? | What To Do | Why |
|---------------|---------|-----------|------------|-----|
| **Period name** | "Roman Republic" | âœ… YES | Pass to LLM for extraction | Natural language, won't tokenize destructively |
| **Place name** | "Rome" | âœ… YES | Pass to LLM for extraction | Natural language, won't tokenize destructively |
| **Subject heading** | "Political science" | âœ… YES | Pass to LLM for classification | Natural language phrase |
| **Person name** | "Julius Caesar" | âœ… YES | Pass to LLM for extraction | Natural language name |
| **Wikidata QID** | "Q17193" | âŒ NO | Use tool lookup only | Tokenizes to [Q, 17, 19, 3] â†’ lookup fails |
| **FAST ID** | "1145002" | âŒ NO | Use tool lookup only | Tokenizes to [114, 500, 2] â†’ backbone broken |
| **LCC code** | "DG241-269" | âŒ NO | Use tool lookup only | Tokenizes to [DG, 241, -, 269] â†’ classification fails |
| **MARC code** | "sh85115058" | âŒ NO | Use tool lookup only | Tokenizes to [sh, 851, 150, 58] â†’ lookup fails |
| **Pleiades ID** | "423025" | âŒ NO | Use tool lookup only | Tokenizes to [423, 025] â†’ ancient geo breaks |
| **ISO 8601 date** | "-0753-01-01" | âŒ NO | Store only, never analyze | Tokenizes to [-, 075, 3, -, 01, -, 01] â†’ date parsing breaks |

**Golden Rule:** If it's an atomic identifier (not natural language), NEVER pass it to an LLM for interpretation.

---

## **M.2 Common Patterns: Correct vs. Wrong**

### **Pattern 1: Entity Extraction**

**âŒ WRONG: Asking LLM to work with identifiers**
```python
# NEVER DO THIS
prompt = f"What period is Wikidata QID Q17193?"
response = llm.ask(prompt)  # âŒ Q17193 tokenizes to [Q, 17, 19, 3]
```

**âœ… CORRECT: Two-stage processing**
```python
# Stage 1: LLM extracts natural language
prompt = "Extract the historical period mentioned: 'During the Roman Republic...'"
extracted = llm.extract(prompt)  # Returns: {"period": "Roman Republic"}

# Stage 2: Tool resolves to identifier (NO LLM)
qid = wikidata_tool.lookup("Roman Republic")  # Returns: "Q17193" (atomic string)
fast_id = fast_tool.lookup("Roman Republic")   # Returns: "1411640" (atomic string)

# Stage 3: Store both formats
entity = {
    "label": "Roman Republic",  # âœ… Natural language (for LLMs)
    "qid": "Q17193",            # âŒ Atomic (NEVER pass to LLM)
    "fast_id": "1411640"        # âŒ Atomic (NEVER pass to LLM)
}
```

---

### **Pattern 2: Subject Classification**

**âŒ WRONG: Asking LLM to generate FAST IDs**
```python
# NEVER DO THIS
prompt = "What is the FAST ID for 'technology'?"
response = llm.ask(prompt)  # âŒ LLM will hallucinate an ID
```

**âœ… CORRECT: LLM classifies natural language, tool converts**
```python
# Stage 1: LLM classifies into natural language category
prompt = "Classify this text into a subject area: 'The invention of the printing press...'"
extracted = llm.classify(prompt)  # Returns: "Technology and civilization"

# Stage 2: Tool looks up FAST ID (NO LLM)
fast_id = fast_api.lookup("Technology and civilization")  # Returns: "1145002"

# Stage 3: Store both
subject = {
    "label": "Technology and civilization",  # âœ… For display
    "fast_id": "1145002"                    # âŒ NEVER pass to LLM
}
```
...
**âŒ WRONG: Asking LLM to interpret ISO 8601 dates**
```python
# NEVER DO THIS
date = "-0509-01-01"  # Founding of Roman Republic
prompt = f"What year is {date}?"
response = llm.ask(prompt)  # âŒ Date tokenizes to [-, 050, 9, -, 01, -, 01]
```

**âœ… CORRECT: LLM extracts natural language, tool formats**
```python
# Stage 1: LLM extracts natural language date
prompt = "Extract the date: 'The Roman Republic was founded in 509 BCE'"
extracted = llm.extract(prompt)  # Returns: {"year": "509 BCE"}

# Stage 2: Tool converts to ISO 8601 (NO LLM)
iso_date = date_tool.to_iso8601("509 BCE")  # Returns: "-0509-01-01"

# Stage 3: Store both
temporal = {
    "label": "509 BCE",          # âœ… For display
    "start_date": "-0509-01-01"  # âŒ NEVER pass to LLM
}
```

---

## **M.3 Tokenization Examples: Why Identifiers Fragment**

### **Example 1: FAST ID Fragmentation**

```python
# Identifier: "1145002" (Technology and civilization)
fast_id = "1145002"

# What LLM sees when tokenized:
tokens = ["114", "500", "2"]  # âŒ Three separate numbers!

# Result: Lookup fails because "114", "500", "2" != "1145002"
```

**Impact:** FAST backbone alignment fails â†’ subject classification breaks

---

### **Example 2: LCC Code Fragmentation**

```python
# Identifier: "DG241-269" (Ancient Italy, Rome to 476)
lcc_code = "DG241-269"

# What LLM sees when tokenized:
tokens = ["DG", "241", "-", "269"]  # âŒ Four separate tokens!

# Result: Classification lookup fails
```

**Impact:** LCC routing to specialist agents fails â†’ wrong agent handles query

---

### **Example 3: Wikidata QID Fragmentation**

```python
# Identifier: "Q17193" (Roman Republic)
qid = "Q17193"

# What LLM sees when tokenized:
tokens = ["Q", "17", "19", "3"]  # âŒ Four tokens instead of one!

# Result: Entity resolution fails
```

**Impact:** Wikidata lookups fail â†’ entity linking breaks

---

### **Example 4: ISO 8601 Date Fragmentation**

```python
# Date: "-0509-01-01" (509 BCE, founding of Roman Republic)
iso_date = "-0509-01-01"

# What LLM sees when tokenized:
tokens = ["-", "050", "9", "-", "01", "-", "01"]  # âŒ Seven tokens!

# Result: Date interpretation fails
```

**Impact:** Temporal queries fail â†’ timeline broken

---

## **M.4 Storage Format Examples**

### **Period Entity (Dual Format Storage)**

```json
{
  "entity_id": "per_000456",
  "entity_type": "Period",
  
  "label": "Roman Republic",               // âœ… LLM-safe (natural language)
  "name": "Roman Republic",                // âœ… LLM-safe
  
  "qid": "Q17193",                         // âŒ ATOMIC (tool only)
  "fast_id": "1411640",                    // âŒ ATOMIC (tool only)
  "periodo_uri": "p0tmp",                  // âŒ ATOMIC (tool only)
  
  "start_date": "-0509-01-01",             // âŒ ATOMIC (store only)
  "end_date": "-0027-01-01",               // âŒ ATOMIC (store only)
  "date_precision": "year",                // âœ… LLM-safe enum
  
  "description": "Period of ancient Roman civilization..."  // âœ… LLM-safe
}
```

**Usage Rules:**
- **LLM prompts**: Use `label`, `name`, `description` only
- **Tool lookups**: Use `qid`, `fast_id`, `periodo_uri` only
- **Storage**: Store both formats for maximum utility

---

### **Place Entity (Geographic Identifiers)**

```json
{
  "entity_id": "plc_000789",
  "entity_type": "Place",
  
  "label": "Rome",                         // âœ… LLM-safe (natural language)
  "name": "Rome",                          // âœ… LLM-safe
  
  "qid": "Q220",                           // âŒ ATOMIC (tool only)
  "pleiades_id": "423025",                 // âŒ ATOMIC (tool only)
  "tgn_id": "7000874",                     // âŒ ATOMIC (tool only)
  "geonames_id": "3169070",                // âŒ ATOMIC (tool only)
  
  "latitude": 41.9028,                     // âœ… Safe (numeric)
  "longitude": 12.4964,                    // âœ… Safe (numeric)
  
  "place_type": "city",                    // âœ… LLM-safe enum
  "modern_name": "Rome, Italy"             // âœ… LLM-safe
}
```

**Usage Rules:**
- **LLM prompts**: Use `label`, `name`, `modern_name`, `place_type`
- **Tool lookups**: Use `qid`, `pleiades_id`, `tgn_id`, `geonames_id`
- **Coordinates**: Numeric values are safe (won't fragment)
...
```cypher
// Relationship edge with action structure
(:Human {label: "Julius Caesar", qid: "Q1048"})-
  [:COMMANDED {
    action_type: "military_command",     // âœ… LLM-safe vocabulary code
    action_code: "A.2.1",                // âŒ ATOMIC (tool only)
    goal_type: "territorial_expansion",  // âœ… LLM-safe vocabulary code
    goal_code: "G.1.3",                  // âŒ ATOMIC (tool only)
    confidence: 0.95,                    // âœ… Safe (numeric)
    source_work_qid: "Q2394"             // âŒ ATOMIC (tool only)
  }]->
(:Organization {label: "Legio X Gemina", qid: "Q158885"})
```

**Usage Rules:**
- **LLM prompts**: Use natural language properties (`action_type`, `goal_type`)
- **Tool navigation**: Use code properties (`action_code`, `goal_code`, `source_work_qid`)

---

## **M.5 Validation Checklist (Pre-LLM Call)**

Before sending ANY prompt to an LLM, check for:

1. âš ï¸ **Wikidata QIDs** (pattern: `Q` followed by digits)
   - âŒ Bad: `"What is Q17193?"`
   - âœ… Good: `"What is the Roman Republic?"`

2. âš ï¸ **FAST IDs** (pattern: 7-digit number)
   - âŒ Bad: `"Classify using FAST ID 1145002"`
   - âœ… Good: `"Classify into subject 'Technology and civilization'"`

3. âš ï¸ **LCC codes** (pattern: letters + numbers + optional range)
   - âŒ Bad: `"Is this in LCC range DG241-269?"`
   - âœ… Good: `"Is this about ancient Rome?"`

4. âš ï¸ **MARC codes** (pattern: `sh` + 8 digits)
   - âŒ Bad: `"Look up MARC sh85115058"`
   - âœ… Good: `"Look up subject heading 'Technology'"`

5. âš ï¸ **Pleiades IDs** (pattern: 6-digit number)
   - âŒ Bad: `"What is Pleiades 423025?"`
   - âœ… Good: `"What is the ancient location 'Rome'?"`

6. âš ï¸ **ISO 8601 dates with negative years** (pattern: `-YYYY-MM-DD`)
   - âŒ Bad: `"When is -0509-01-01?"`
   - âœ… Good: `"When was 509 BCE?"`

7. âš ï¸ **TGN IDs** (pattern: 7-digit number)
   - âŒ Bad: `"TGN 7000874 location"`
   - âœ… Good: `"The location 'Rome'"`

8. âš ï¸ **GeoNames IDs** (pattern: 7-digit number)
   - âŒ Bad: `"GeoNames 3169070"`
   - âœ… Good: `"The modern city of Rome"`

**Automated Check:**
```python
validator = IdentifierValidator()  # From Section 8.5.6
result = validator.check_prompt(prompt)

if not result["is_safe"]:
    raise ValueError(f"Prompt contains atomic identifiers: {result['issues']}")
```

---

## **M.6 Emergency Decision Tree**

```
Is this natural language (e.g., "Roman Republic", "Julius Caesar")?
â”œâ”€ YES â†’ âœ… Safe to pass to LLM
â””â”€ NO â†’ Is it an identifier (QID, FAST, LCC, etc.)?
    â”œâ”€ YES â†’ âŒ DO NOT pass to LLM. Use tool lookup.
    â””â”€ UNSURE â†’ âš ï¸ Default to tool handling (safer to over-protect)
```

**When in doubt:** Always use tool-based lookups instead of LLM processing.

---

## **M.7 Anti-Patterns Summary**

### âŒ **Anti-Pattern 1: Passing QID to LLM**
```python
# WRONG
prompt = f"What is the entity with QID {qid}?"
```

### âŒ **Anti-Pattern 2: Asking LLM to Generate FAST IDs**
```python
# WRONG (LLM will hallucinate)
prompt = "Generate a FAST ID for 'technology'"
```

### âŒ **Anti-Pattern 3: Constructing Prompts with Identifiers**
```python
# WRONG
prompt = f"Compare FAST {id1} and FAST {id2}"
```

### âœ… **Correct Pattern: Two-Stage Processing**
```python
# Stage 1: LLM extracts natural language labels
# Stage 2: Tools resolve labels â†’ atomic IDs
# Stage 3: Store both formats (label + ID)
```

---

## **M.8 Reference Implementation**

See Section 8.5 for complete implementation details:
- Section 8.5.1: The Tokenization Problem
- Section 8.5.2: Two-Stage Processing Pattern
- Section 8.5.3: Identifier Atomicity Rules
- Section 8.5.4-8.5.8: Implementation patterns and examples
- Section 8.5.6: `IdentifierValidator` class (Python code)
...
This appendix documents major architectural decisions, their context, rationale, and consequences. Each ADR explains why specific design choices were made over alternatives.

---

## **ADR-001: Two-Stage Architecture (LLM Extraction â†’ Reasoning Validation)**

**Status:** Adopted  
**Date:** Early 2025  
**Context:** Need to balance LLM extraction capabilities with deterministic validation

**Decision:** Separate LLM extraction stage from deterministic reasoning validation stage

**Rationale:**
- LLMs excel at extraction but hallucinate
- Reasoning models provide verification but can't extract
- Separation enables leveraging strengths while mitigating weaknesses

**Consequences:**
- Explicit provenance at every stage
- Human review possible at validation boundaries
- Schema compliance enforced deterministically
- Authority matching verified before Neo4j write

**See:** Section 1.2.1 (Architectural Principles)

---

## **ADR-002: Structure vs. Topics Separation (FAST Principles)**

**Status:** Adopted  
**Date:** December 2025  
**Source:** ONTOLOGY_PRINCIPLES.md

**Context:** Historical events can be "about" multiple topics. Need to organize knowledge without redundant subject hierarchies.

**Decision:** Separate **structure** (LCC classification) from **topics** (FAST headings)

**Rationale:**
- **Structure (LCC):** Organizational path - ONE primary classification per entity
- **Topics (FAST):** Semantic aboutness - MANY topics per entity
- Prevents redundant hierarchies (no need for both "Political Events" AND "Military Events" trees)
- Aligns with library science principles

**Example:**
```
Julius Caesar entity:
  - Structure: DG261.C35 (Roman biography - single path)
  - Topics: [Roman Civil War, Generals, Statesmen] (multiple subjects)
```

**Consequences:**
- Entities have ONE `backbone_lcc` (classification)
- Entities have MULTIPLE `backbone_fast` (topics)
- Enables faceted search across multiple dimensions
- Supports multiple perspectives on same entity

**See:** Section 1.2.3 (Architectural Principles)

---

## **ADR-003: LCC Instead of Dewey for Classification Backbone**

**Status:** Adopted  
**Date:** December 2025  
**Source:** LCC_AGENT_ROUTING.md

**Context:** Need primary classification system for agent routing and organization

**Decision:** Use Library of Congress Classification (LCC) as primary backbone
...
**Example:**
- LCC: `DG261.C35` (Julius Caesar biography - specific)
- Dewey: `937.05092` (Roman Republic biography - generic)

**Consequences:**
- Agent routing uses LCC codes
- Enables fine-grained subject specialization
- Compatible with academic library catalogs
- Dewey available as secondary classification if needed

**See:** Section 1.4.1 (Key Design Decisions)

---

## **ADR-004: Two-Level Agent Granularity (FAST + LCC)**

**Status:** Adopted  
**Date:** December 2025  
**Source:** Subject_Agent_Granularity_Strategy.md

**Context:** Balance between agent expertise and agent proliferation

**Problem:**
- Too coarse: "Ancient History Agent" lacks domain expertise
- Too fine: "Julius Caesar Specialist Agent" causes agent proliferation (1000s of agents)

**Decision:** Two-level hierarchy:
- **Level 1:** FAST broad topics (22 agents)
- **Level 2:** LCC subdivisions (dynamic routing within agents)

**Example Routing:**
```
User: "Tell me about Caesar's military tactics"
  â†’ FAST: fst01411640 (Roman Civil War) â†’ Roman History Agent
    â†’ LCC: DG261.C35 (Caesar biography) â†’ RAG query within agent's scope
      â†’ Specific tactics from agent's vector store
```

**Consequences:**
- Manageable agent count (22 specialists vs. 1000s)
- Maintains domain expertise (not generic "ancient history")
- Hierarchical routing enables scale
- Each agent maintains subject-scoped RAG index

**See:** Section 1.4.2 (Key Design Decisions), Section 5.4 (Agent Granularity)

---

## **ADR-005: CIDOC-CRM Extension Over Parallel System**

**Status:** Adopted  
**Date:** February 2026  
**Source:** CIDOC-CRM_vs_Chrystallum_Comparison.md

**Context:** CIDOC-CRM (ISO 21127:2023) exists for cultural heritage. Should Chrystallum build on it or create parallel system?

**Decision:** Use CIDOC-CRM as **foundation**, extend with Chrystallum features

**What CIDOC-CRM Provides:**
- Event-centric model (E5_Event, E21_Person, E53_Place)
- Temporal/spatial relationships (P4_has_time-span, P7_took_place_at)
- Museum interoperability (ISO standard, RDF/OWL compatible)

**What CIDOC-CRM Does NOT Provide:**
- Library backbone standards (FAST, LCC, LCSH, MARC)
- Systematic ISO 8601 historical dates (BCE support)
- Action structure vocabularies (goal/trigger/action/result)
- Wikidata alignment built-in
- Agent-based construction methodology
...
**Consequences:**
- Museum/archival compatibility via CIDOC-CRM
- Historical research features via Chrystallum extensions
- Best of both worlds
- Can add CIDOC-CRM temporal patterns (P4_has_time-span) if museum integration needed

**See:** Section 1.2.4 (CIDOC-CRM Relationship), Appendix L (CIDOC-CRM Integration Guide)

---

## **ADR-006: Hybrid Architecture - Entities (Traversal) + Claims (Content-Addressable)** ðŸ”¥ NEW

**Status:** Adopted  
**Date:** February 12, 2026  
**Context:** Need to balance entity exploration (traversal) with evidence verification (content-addressable)

### **Decision:** Use **hybrid architecture**:
- **Base entities:** Neo4j property graph with traditional Cypher traversal
- **Claims layer:** Content-addressable via `cipher = Hash(claim structure)`

### **Rationale:**

**Different Query Patterns:**

| Layer | Query Type | Pattern |
|-------|-----------|---------|
| **Entities** | Exploration, relationship discovery | Traversal (Cypher MATCH) |
| **Claims** | Verification, deduplication, attachment | Direct cipher lookup |

**Entity Layer Needs:**
- "What connects Caesar to Cleopatra?" â†’ Traversal finds paths
- "Find all senators who opposed Caesar" â†’ Pattern matching
- Graph algorithms (PageRank, centrality, community detection)
- **Traversal provides flexibility**

**Claims Layer Needs:**
- "Have we seen this evidence before?" â†’ Cipher deduplication
- "Has this claim been tampered with?" â†’ Cryptographic verification
- "What exact source supports this?" â†’ Content-addressable integrity
- **Cipher provides verification**

### **Claim as Subgraph Cluster:**

A claim isn't a single nodeâ€”it's a **complex evidence structure**:

```cypher
// Cipher identifies entire subgraph
(claim:Claim {cipher: "claim_abc123..."})

// Components attached
(source:Work)-[:EXTRACTED_FROM]->(claim)
(passage:Passage)-[:CITED_BY]->(claim)
(entity1)-[:SUBJECT_OF]->(claim)
(entity2)-[:OBJECT_OF]->(claim)
(relationship)-[:ASSERTED_BY]->(claim)
(agent:Agent)-[:EXTRACTED_BY]->(claim)
```

**The cipher is the cluster key** - unambiguous attachment point for:
- Frameworks (W5H1 analysis)
- Reviews (validation decisions)
- Beliefs (confidence-weighted conclusions)

### **Academic Use Cases:**

**Scenario: Multiple Scholars Extract Same Claim**

```python
# Scholar A extracts from Plutarch at 10:00 AM
claim_data_A = {
    "source": "Q644312",
    "passage": "Caesar crossed the Rubicon...",
    "subject": "Q1048",
...
# Scholar B extracts SAME claim at 2:00 PM
claim_data_B = {
    "source": "Q644312",
    "passage": "Caesar crossed the Rubicon...",
    "subject": "Q1048",
...
if cipher_B in graph:
    add_supporting_review(cipher_B, scholar_B)
    increment_confidence(cipher_B)
else:
    create_new_claim(cipher_B)
```

**No duplicate claims** - multiple validations increase confidence instead.

### **Distributed Academic Knowledge:**

```python
# University A creates claim
cipher = Hash(claim_data)
state_root = MerkleRoot([cipher, sources, entities])
...
# University B verifies and cites
downloaded = fetch_claim(cipher)
if verify_cipher(downloaded):
    # âœ… Cryptographically verified
    cite_with_proof(cipher, state_root)
```

**Enables P2P academic networks** with verifiable citations.
...
CREATE (claim:Claim {
  // Content-addressable ID (primary)
  cipher: "claim_kc1-b22020c0e271b7d...",
  
  // Components for cipher generation
  source_work_qid: "Q644312",
  passage_hash: Hash("Caesar crossed..."),
  subject_entity_qid: "Q1048",
  relationship_type: "CAUSED",
  confidence: 0.95,
  extractor_agent_id: "genericagent...",
  extraction_timestamp: "2026-02-12T10:30:00Z",
...
**Positive:**
- âœ… Automatic claim deduplication (same evidence = same cipher)
- âœ… Cryptographic integrity verification (hash proves claim unchanged)
- âœ… Deterministic IDs (same claim by different agents = same cipher)
- âœ… Unambiguous framework attachment (cipher = cluster key)
- âœ… Enables distributed academic citations with verifiable provenance
- âœ… Built-in versioning (modified claim = different cipher + SUPERSEDES link)

**Trade-offs:**
- âš ï¸ Claims must be immutable (changing data â†’ different cipher)
- âš ï¸ Need version chains for claim evolution (SUPERSEDES relationships)
- âš ï¸ Cipher computation adds overhead (but negligible vs. benefits)
- âš ï¸ Entities remain mutable (different purpose than claims)

### **Why Not Content-Addressable for Everything?**

**Entities need to evolve:**
- Historical scholarship discovers new information
- Properties are refined over time
- Names have variants and translations
- **Mutability is necessary**

**Claims need to be immutable:**
- Evidence is fixed at extraction time
- Verification requires stable content
- Provenance chains demand immutability
- **Immutability enables verification**
...
**Why the change:**
- Entities need relationship discovery (traversal wins)
- Claims need verification (content-addressable wins)
- Hybrid approach gets best of both

**See:** Section 6.4 (Content-Addressable Claim Identification)

---

## **ADR-007: Calendar Normalization for Historical Dates**

**Status:** Adopted  
**Date:** December 2025  
**Source:** Historical_Dating_Schema_Disambiguation.md

**Context:** Historical sources use different calendar systems (Julian, Gregorian, Roman AUC)

**Problem:**
- Julian calendar: "October 5, 1582"
- Gregorian calendar: "October 15, 1582" (same day!)
- Without normalization: System sees date conflicts â†’ false confidence degradation

**Decision:** Store **normalized canonical dates** + **original source dates** + **calendar metadata**

**Schema:**
```cypher
(event:Event {
  start_date: "1582-10-05",           // Canonical (normalized)
  start_date_original: "1582-10-05",  // Source value
  calendar_system: "Julian",          // Metadata
  date_precision: "day"
})
...
- Post-1582-10-15: Assume Gregorian unless stated
- BCE dates: Always ISO 8601 negative years (`-0049` for 49 BCE)
- Store both original + normalized for provenance

**Consequences:**
- âœ… Prevents false confidence degradation from calendar mismatches
- âœ… Enables accurate temporal queries across calendar systems
- âœ… Preserves source fidelity (original dates retained)
- âœ… Systematic ISO 8601 format throughout system

**See:** Section 1.4.3 (Calendar Normalization), Section 3.6 (Temporal Modeling)

---

(Additional ADRs would follow as architecture evolves...)

---

(Content continues with remaining appendices...)
