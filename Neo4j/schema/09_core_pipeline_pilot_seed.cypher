...
MERGE (sc:SubjectConcept {subject_id: 'subj_roman_republic_001'})
...
MERGE (a:Agent {agent_id: 'agent_roman_republic_v1'})
...
MATCH (a:Agent {agent_id: 'agent_roman_republic_v1'})
MATCH (sc:SubjectConcept {subject_id: 'subj_roman_republic_001'})
MERGE (a)-[:OWNS_DOMAIN]->(sc);

// 3) Core claim
MERGE (c:Claim {claim_id: 'claim_roman_republic_end_27bce_001'})
SET c.cipher = '8a8f9e79c1f271d90a0afad7d7d1f3e2b7e572f3bb8db4c8e0a4215ef3a27f12',
    ...
    c.source_agent = 'agent_roman_republic_v1',
...
MATCH (a:Agent {agent_id: 'agent_roman_republic_v1'})
MATCH (sc:SubjectConcept {subject_id: 'subj_roman_republic_001'})
MATCH (c:Claim {claim_id: 'claim_roman_republic_end_27bce_001'})
MERGE (a)-[:MADE_CLAIM]->(c)
MERGE (sc)-[:SUBJECT_OF]->(c);

// 4) Retrieval context (provenance container)
MERGE (rc:RetrievalContext {retrieval_id: 'retr_roman_republic_q17167_001'})
SET rc.agent_id = 'agent_roman_republic_v1',
    rc.timestamp = toString(datetime()),
    rc.source = 'wikidata',
    rc.seed_qid = 'Q17167';

MATCH (c:Claim {claim_id: 'claim_roman_republic_end_27bce_001'})
MATCH (rc:RetrievalContext {retrieval_id: 'retr_roman_republic_q17167_001'})
MERGE (c)-[:USED_CONTEXT]->(rc);

// 5) Analysis run + one facet assessment
MERGE (run:AnalysisRun {run_id: 'run_roman_republic_001'})
SET run.pipeline_version = 'v1.0',
    run.status = 'completed',
    run.created_at = toString(datetime());

MATCH (c:Claim {claim_id: 'claim_roman_republic_end_27bce_001'})
MATCH (run:AnalysisRun {run_id: 'run_roman_republic_001'})
MERGE (c)-[:HAS_ANALYSIS_RUN]->(run);

MERGE (f:Facet {facet_id: 'facet_political', label: 'Political'});

MERGE (fa:FacetAssessment {assessment_id: 'fa_roman_republic_pol_001'})
SET fa.score = 0.95,
    fa.status = 'supported',
...
MATCH (run:AnalysisRun {run_id: 'run_roman_republic_001'})
MATCH (fa:FacetAssessment {assessment_id: 'fa_roman_republic_pol_001'})
