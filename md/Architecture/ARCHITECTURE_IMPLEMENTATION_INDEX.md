...
- In progress: GeoNames crosswalk and feature-code alignment (`Pleiades+ -> GeoNames`), GeoNames->Wikidata bridge (`P1566`)
- Planned/TODO (architecture-defined): Trismegistos, EDH, VIAF (direct adapters), federation meta-subgraph seed
...
**Recent Updates:**
- **2026-02-19:** Added GeoNames->Wikidata federation bridge (`P1566`) and merged `Pleiades+GeoNames+Wikidata+TGN` crosswalk outputs via `build_geonames_wikidata_bridge.py`
- **2026-02-19:** Added federation triangulation + Chrystallum federation meta-subgraph TODO pack (`FEDERATION_GOLDEN_PATTERN_TODO_2026-02-19.md`) and queued execution backlog items (`ARB-FED-002..006`, `ARB-GEO-001`)
- **2026-02-18:** GNN experiment runner scaffold (`ARB-GNN-002`) added with non-production metadata output
- **2026-02-18:** GNN experiment protocol (`ARB-GNN-001`) added as non-production research track baseline
- **2026-02-18:** Federation bidirectional query design (`ARB-FED-001`) and embedding-assisted disambiguation policy (`ARB-ER-001`) added
- **2026-02-18:** Schema migration regression runner (`ARB-SCHEMA-002`) and migration plan v1 added; vector index integration plan (`ARB-EMB-003`) drafted
- **2026-02-18:** Schema migration regression harness design added (`ARB-SCHEMA-001`) with pre/post fixed assertion strategy
- **2026-02-18:** Semantic validator runner scaffold added (`ARB-REAS-002`) with deterministic JSON report output
- **2026-02-18:** SHACL/RDFS-lite scope doc and embedding pilot scaffold added (`ARB-REAS-001`, `ARB-EMB-002`)
- **2026-02-18:** External architecture review execution backlog added with phased task IDs and acceptance criteria
- **2026-02-18:** External architecture review response added with explicit accept/challenge/defer matrix and phased execution plan
