...
```
Entity (Wikidata QID)
  → Wikidata P1047 → LGPN ID (format: V1-12345, V2-67890, etc.)
...
> **⚠ SUPERSEDED (2026-02-24):** The framing below — federation data written as
> metadata fields on entity nodes — has been superseded by `FEDERATION_ARCHITECTURE.md`.
> Federation records are now first-class graph claims with PROV-O provenance,
> not metadata fields. The pipeline structure below remains accurate but the
> output format has changed. See `FEDERATION_ARCHITECTURE.md` for the current design.

The pipeline runs in two phases:

**Phase 1 — Gate (current immediate work):**
```
find_anchors → validate → harvest → facet_classify → cluster_assign
                                                          ↓
                                          federation ID present? → scoped/unscoped
```

**Phase 2 — Source (subsequent enrichment pass):**
```
scoped entity layer
        ↓
federation interrogation (Trismegistos, LGPN, Pleiades APIs)
        ↓
first-class graph claims written with source: trismegistos/lgpn/pleiades
        ↓
SCA re-scores with enriched entity layer (bootstrap_readiness: PHASE2_COMPLETE)
```

The enrichment is opportunistic — it only fires on entities where Wikidata
carries P1696 (TM ID) or P1047 (LGPN ID) or P1584 (Pleiades ID). Coverage will
be partial, concentrated on named individuals and places where cross-authority
records exist. That is the right coverage: exactly where primary source depth
adds most value over Wikidata alone.
