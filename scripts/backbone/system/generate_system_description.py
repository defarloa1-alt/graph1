...
Usage:
    ...
    python scripts/backbone/system/generate_system_description.py \\
...
# Load config (Neo4j, API keys) from .env
try:
    _scripts = Path(__file__).resolve().parents[2]  # scripts/
    sys.path.insert(0, str(_scripts))
    from config_loader import (
        NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD,
        PERPLEXITY_API_KEY, get_config,
    )
except ImportError:
    NEO4J_URI = NEO4J_USERNAME = NEO4J_PASSWORD = None
    PERPLEXITY_API_KEY = None
    get_config = lambda k, d=None: None
...
QUERIES = {
    ...
    "federations": """
        ...
        RETURN
          f.name             AS name,
          f.status           AS type,
          f.scoping_role     AS mode,
          f.license          AS license,
          f.phase1_complete  AS phase1_complete,
          f.phase2_complete  AS phase2_complete,
          f.confidence       AS confidence,
          f.wikidata_property AS wikidata_property
    ...
    "entities_summary": """
        ...
        RETURN
          ...
          count(CASE WHEN e.primary_facet = 'BIOGRAPHIC' THEN 1 END)   AS biographical,
        ...
        with driver.session() as session:
            for key, query in QUERIES.items():
                try:
                    ...
                    results[key] = records
                    print(f"  [OK] {key}: {len(records)} record(s)")
...
def assemble_structured_description(raw: dict) -> dict:
    ...
    for f in feds:
        ftype = f.get("type", "unknown")  # status: operational/planned/partial
        fed_by_type.setdefault(ftype, []).append({
            "name":     f.get("name"),
            "mode":     f.get("mode"),      # scoping_role
            "coverage": None,
    ...
    description = {
        ...
        "entities": {
            "total":     total_entities,
            "by_facet": {
                "BIOGRAPHIC":     ent.get("biographical", 0),
...
def write_description(
    ...
    try:
        with driver.session() as session:
            ...
            if result:
                r = result[0]
                print(f"  [OK] SystemDescription written")
...
def main():
    ...
    parser.add_argument("--anthropic-api-key",  default=get_config("ANTHROPIC_API_KEY") if get_config else None,
        help="Anthropic API key (for --llm claude)")
    parser.add_argument("--perplexity-api-key", default=PERPLEXITY_API_KEY or (get_config("PPLX_API_KEY") if get_config else None),
    ...
    if not args.neo4j_uri:
        parser.error("--neo4j-uri required (or set NEO4J_URI in .env)")
