  ...
  layers: [
    {
      ...
      node_types:["Entity","Discipline","Facet","SYS_FederationSource","LCSH_Heading",
        "LCC_Class","WorldCat_Work","BibliographySource","TextPassage","RetrievalContext","SYS_RelationshipType"],
...
const REPOS = {
  ...
  INTERNET_ARCHIVE:  { name:"Internet Archive (subject)", color:C.amber,
    url:"https://archive.org/advancedsearch.php?q=subject%3A{label}+AND+mediatype%3Atexts&fl[]=identifier,title,creator,date&output=json&rows=50" },
  IA_CORPUS_SCRAPE:  { name:"IA Scrape API",      color:C.amber,
    url:"https://archive.org/services/search/v1/scrape?fields=identifier,title,creator,subject,date&q=(collection:universallibrary+OR+collection:gutenberg)+AND+subject:{label}&count=100",
    note:"Cursor-based bulk harvest, no 10k limit. Add sin=TXT to shift from metadata to full-text search across all IA OCR." },
  IA_SEARCHINSIDE:   { name:"IA Search Inside",   color:C.crimson,
    url:"https://api.archivelab.org/books/{ia_identifier}/searchinside?q={term}",
    note:"Per-item. Returns: page number + surrounding passage + bounding box coords. ia_identifier is new property on :BibliographySource. Use term = person/office/place name." },
  IA_FULLTEXT:       { name:"IA Full Text (djvu)", color:C.crimson,
    url:"https://archive.org/download/{ia_identifier}/{ia_identifier}_djvu.txt",
    note:"Complete OCR text. Deterministic URL from ia_identifier. Harvest → chunk → :TextPassage nodes with APPEARS_IN :BibliographySource + MENTIONS :Entity." },
  IA_PAGES:          { name:"IA Pages API",        color:C.rose,
    url:"https://api.archivelab.org/books/{ia_identifier}/pages/{page_num}",
    note:"Returns scanned page as .jpg. Pair with searchinside results to surface the actual page in UI." },
  GUTENBERG_TEXT:    { name:"Gutenberg Plain Text", color:C.navy,
    url:"https://www.gutenberg.org/ebooks/{gutenberg_id}.txt.utf-8",
    note:"Canonical UTF-8 plain text. Pattern is deterministic from gutenberg_id property on :BibliographySource." },
  GUTENDEX:          { name:"Gutendex API",         color:C.navy,
    url:"https://gutendex.com/books?topic={slug}&languages=en&sort=popular",
    note:"JSON catalogue of 76k+ Gutenberg books. Filter author_year_end=-499 for ancient authors. Returns formats{} with direct text/epub download URLs." },
...
const FEDERATION_SOURCES = [
  ...
  { name:"IA Scrape API",    status:"planned",     weight:0.88, role:"corpus_discovery",
    entity_types:["BibliographySource","Discipline"], prop:null, access:"api", format:"JSON",
    endpoint:"https://archive.org/services/search/v1/scrape",
    url_template:"https://archive.org/services/search/v1/scrape?fields=identifier,title,creator,subject,date&q=(collection:universallibrary+OR+collection:gutenberg)+AND+subject:{label}&count=100",
    node_type:"BibliographySource", prop_name:"ia_identifier",
    notes:"Cursor-based, no 10k limit. Add &sin=TXT to search full OCR text (not just metadata) across all texts simultaneously — cross-corpus term search." },
  { name:"IA Search Inside",  status:"planned",     weight:0.92, role:"primary_texts",
    entity_types:["BibliographySource","Entity"], prop:null, access:"api", format:"JSON",
    endpoint:"https://api.archivelab.org/books/{ia_identifier}/searchinside",
    url_template:"https://api.archivelab.org/books/{ia_identifier}/searchinside?q={term}",
    node_type:"TextPassage", prop_name:"ia_identifier",
    notes:"Per-item. Returns page + surrounding passage + bounding box. ia_identifier on :BibliographySource. Seed items confirmed: Livy (universallibrary), Caesar Gallic War, Polybius (Gutenberg), Plutarch Lives (Gutenberg). Passages → :TextPassage nodes → MENTIONS :Entity." },
  { name:"IA Full Text",      status:"planned",     weight:0.90, role:"primary_texts",
    entity_types:["BibliographySource"], prop:null, access:"direct", format:"plaintext",
    endpoint:"https://archive.org/download/{ia_identifier}/{ia_identifier}_djvu.txt",
    url_template:"https://archive.org/download/{ia_identifier}/{ia_identifier}_djvu.txt",
    node_type:"TextPassage", prop_name:"ia_identifier",
    notes:"Deterministic URL from ia_identifier. Full OCR text. Chunk into :TextPassage (paragraph or fixed 400-char window). Back-of-book index entries captured in OCR — index headings surfaceable via searchinside." },
  { name:"Gutendex",          status:"planned",     weight:0.82, role:"corpus_discovery",
    entity_types:["BibliographySource","Discipline"], prop:null, access:"api", format:"JSON",
    endpoint:"https://gutendex.com/books",
    url_template:"https://gutendex.com/books?topic={slug}&languages=en&sort=popular",
    node_type:"BibliographySource", prop_name:"gutenberg_id",
    notes:"JSON catalogue of all 76k+ Gutenberg books. formats{} field gives direct .txt.utf-8 download URL. Filter author_year_end=-499 for ancient authors. Free, no key required." },
...
// ══════════════════════════════════════════════════════════════════════════════
// IA SEED ITEMS — confirmed primary sources with ia_identifier
// :BibliographySource nodes; ia_identifier enables IA_SEARCHINSIDE + IA_FULLTEXT
// ══════════════════════════════════════════════════════════════════════════════
const IA_SEED_ITEMS = [
  // ── LIVY ─────────────────────────────────────────────────────────────────
  { ia_identifier:"historyofromelit04livyuoft",    collection:"universallibrary",
    title:"The History of Rome (Spillan trans., v.4)", author:"Livy",
    gutenberg_id:null, collection_tag:"universallibrary",
    ocr:"ABBYY", has_index:true, language:"en",
    facets:["Political","Military","Social","Economic","Geographic","Religious"],
    subjects:["Rome -- History"], date:1864,
    notes:"Vol 4 of 4 (Bell/Spillan). Vol 1 identifier: historyofromelit01livyuoft. Includes printed index captured in OCR." },
  { ia_identifier:"livy-history-of-rome-12-volumes-in-1-searchable-loeb",
    collection:"texts", title:"History of Rome — 12 Loeb vols combined (Latin + English)",
    author:"Livy", gutenberg_id:null,
    ocr:"user-upload", has_index:false, language:"la+en",
    facets:["Political","Military","Social","Economic","Geographic"],
    subjects:["Rome -- History"],
    notes:"Latin and English side-by-side in single OCR stream. searchinside query for a Roman name returns both Latin and English renderings in same passage window." },
  { ia_identifier:"historyofrome02livy_1",          collection:"americana",
    title:"History of Rome v.2 (Bell)", author:"Livy",
    gutenberg_id:null, ocr:"ABBYY", has_index:false, language:"en",
    facets:["Political","Military","Social"], subjects:["Rome History"], date:null },
  // Gutenberg Livy — plain text, no IA identifier needed (use gutenberg_id)
  { ia_identifier:null, collection:"gutenberg",
    title:"The History of Rome, Books 01 to 08", author:"Livy",
    gutenberg_id:19725, ocr:null, has_index:false, language:"en",
    facets:["Political","Military","Social","Geographic"],
    subjects:["Rome -- History -- Republic"],
    notes:"Free UTF-8 plain text. https://www.gutenberg.org/ebooks/19725.txt.utf-8" },

  // ── PLUTARCH ─────────────────────────────────────────────────────────────
  { ia_identifier:null, collection:"gutenberg",
    title:"Plutarch's Lives Vol 2 — Marcellus, Marius, Sulla, Lucullus",
    author:"Plutarch", gutenberg_id:14114, ocr:null, has_index:false, language:"en",
    facets:["Biographic","Military","Political","Social"],
    subjects:["Rome -- Biography"],
    notes:"George Long/Stewart trans. Contents: Pelopidas, Marcellus, Aristeides, Cato, Pyrrhus, Marius, Lysander, Sulla, Kimon, Lucullus. PG #14114." },
  // Parallel Lives vols 1, 3, 4 also on Gutenberg — add when needed

  // ── POLYBIUS ─────────────────────────────────────────────────────────────
  { ia_identifier:null, collection:"gutenberg",
    title:"The Histories of Polybius Vol 1 (incl. Book VI — Roman Constitution)",
    author:"Polybius", gutenberg_id:44125, ocr:null, has_index:true, language:"en",
    facets:["Political","Military","Diplomatic","Geographic"],
    subjects:["Rome -- History -- Republic"],
    notes:"Shuckburgh trans. Book VI = Polybian mixed constitution theory — critical for formal_political_theory discipline. Cross-referenced index in Vol 2 (#44126)." },
  { ia_identifier:null, collection:"gutenberg",
    title:"The Histories of Polybius Vol 2", author:"Polybius",
    gutenberg_id:44126, ocr:null, has_index:true, language:"en",
    facets:["Political","Military","Diplomatic","Geographic"],
    subjects:["Rome -- History -- Republic"],
    notes:"Vol 2. Index entries for vol 1 included here. Some index entries contain errors noted in preface." },

  // ── CAESAR ──────────────────────────────────────────────────────────────
  { ia_identifier:"bellumgallicumbo00caes", collection:"universallibrary",
    title:"Bellum Gallicum (Gallic War)", author:"Julius Caesar",
    gutenberg_id:null, ocr:"ABBYY", has_index:true, language:"en",
    facets:["Military","Political","Geographic"],
    subjects:["Gallic War"],
    notes:"djvu.txt confirmed retrievable. Full intro discusses lex annalis, naming conventions (praenomen/nomen/cognomen), dates." },

  // ── SECONDARY (universallibrary confirmed) ────────────────────────────────
  { ia_identifier:"historyoftheroma000914mbp", collection:"universallibrary",
    title:"A History of the Roman Republic", author:"Cyril E. Robinson",
    gutenberg_id:null, ocr:"ABBYY", has_index:false, language:"en",
    facets:["Political","Military","Social","Economic"],
    subjects:["Rome -- History -- Republic"], date:null,
    notes:"RMSC-IIITH scan. 492 pages. Collection: universallibrary. Barcode 104889. Full text + EPUB available." },
];

// ══════════════════════════════════════════════════════════════════════════════
// TEXT PASSAGE NODE SCHEMA
// Neo4j node type: :TextPassage
// Populated by: IA_FULLTEXT harvest (djvu.txt chunked) OR IA_SEARCHINSIDE hits
// ══════════════════════════════════════════════════════════════════════════════
const TEXT_PASSAGE_SCHEMA = {
  label: "TextPassage",
  description: "A located passage of text from a :BibliographySource. Either a pre-chunked paragraph (from full-text harvest) or a search-hit window (from searchinside API). The atomic unit linking corpus text to graph entities.",
  properties: [
    { name:"passage_id",     type:"string",  note:"SHA256 of (ia_identifier + page_num + offset). Stable across re-harvests." },
    { name:"text",           type:"string",  note:"Raw passage text, max ~500 chars. For IA: surrounding text window from searchinside response." },
    { name:"page_num",       type:"integer", note:"Physical page number in source. From searchinside JSON field 'page'. Enables IA_PAGES lookup for image." },
    { name:"char_offset",    type:"integer", note:"Character offset within page OCR text. From searchinside bounding box x/y if available." },
    { name:"source_type",    type:"string",  note:"'searchinside' | 'djvu_chunk' | 'gutenberg_chunk'. Distinguishes harvest method." },
    { name:"query_term",     type:"string",  note:"The term that surfaced this passage (for searchinside hits). Null for full-text chunks." },
    { name:"confidence",     type:"float",   note:"OCR confidence if available from ABBYY hOCR. 0–1." },
    { name:"language",       type:"string",  note:"'en' | 'la' | 'la+en'. For Loeb items, Latin and English may appear in same passage." },
    { name:"created_at",     type:"datetime",note:"Harvest timestamp." },
  ],
  relationships: [
    { type:"APPEARS_IN",   target:":BibliographySource", note:"Many :TextPassage → one :BibliographySource. Carries page_num property." },
    { type:"MENTIONS",     target:":Entity",             note:"Populated by NER pass after harvest. Confidence threshold from SYS_Threshold:mention_confidence." },
    { type:"CITES",        target:":Entity",             note:"Stronger than MENTIONS — passage explicitly refers to entity by name in context of an event or claim." },
    { type:"PART_OF_CHAPTER", target:":TOC_Entry",      note:"If TOC_Entry nodes exist for source, link passage to its chapter/section." },
    { type:"RETRIEVED_FROM", target:":SYS_FederationSource", note:"Which endpoint produced this passage (IA_SEARCHINSIDE or IA_FULLTEXT)." },
  ],
  cypher_create:`// Create :TextPassage from searchinside hit
MERGE (bs:BibliographySource {ia_identifier: $ia_id})
MERGE (tp:TextPassage {passage_id: $passage_id})
  ON CREATE SET
    tp.text         = $text,
    tp.page_num     = $page_num,
    tp.source_type  = 'searchinside',
    tp.query_term   = $query_term,
    tp.language     = $language,
    tp.created_at   = datetime()
MERGE (tp)-[:APPEARS_IN {page_num: $page_num}]->(bs)`,
  cypher_query:`// All passages mentioning a named entity, across all sources
MATCH (e:Entity {label: $name})<-[:MENTIONS]-(tp:TextPassage)-[:APPEARS_IN]->(bs:BibliographySource)
RETURN tp.text, tp.page_num, bs.title, bs.author
ORDER BY bs.date ASC, tp.page_num ASC`,
  harvest_pseudocode:`// Agent harvest loop for a :BibliographySource with ia_identifier
async function harvestEntityMentions(ia_identifier, entity_name) {
  const url = \`https://api.archivelab.org/books/\${ia_identifier}/searchinside?q=\${encodeURIComponent(entity_name)}\`;
  const { results } = await fetch(url).then(r => r.json());
  for (const hit of results) {
    const passage_id = sha256(ia_identifier + hit.page + hit.text.slice(0,40));
    await neo4j.run(CYPHER_CREATE, {
      ia_id: ia_identifier, passage_id,
      text: hit.text, page_num: hit.page,
      query_term: entity_name, language: 'en'
    });
  }
}
// Then: NER pass on all :TextPassage nodes without MENTIONS edges`,
};
...
// ══════════════════════════════════════════════════════════════════════════════
// TEXT PASSAGE & IA SOURCES TAB
// ══════════════════════════════════════════════════════════════════════════════
function TextPassageView() {
  const [section, setSection] = useState("schema");
  const SECTIONS = [["schema","Node Schema"],["seed","Seed Items"],["harvest","Harvest Logic"]];
  return (
    <div>
        ...
        {SECTIONS.map(([k,l])=>(
          <button key={k} onClick={()=>setSection(k)}
            style={{border:"none",padding:"5px 14px",fontSize:9.5,cursor:"pointer",
              ...
              color:section===k?C.crimson:C.dim,
              borderBottom:section===k?`2px solid ${C.crimson}`:"2px solid transparent"}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {section==="schema" && (
        <div>
          <div style={{background:C.crimson+"0A",border:`1.5px solid ${C.crimson}`,
            borderRadius:8,padding:14,marginBottom:14}}>
            <div style={{fontWeight:"bold",color:C.crimson,fontSize:12,marginBottom:4}}>
              :TextPassage
            </div>
            <div style={{fontSize:9,color:C.slate,marginBottom:10}}>{TEXT_PASSAGE_SCHEMA.description}</div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div>
                <div style={{fontSize:8,color:C.dim,fontWeight:"bold",marginBottom:5}}>PROPERTIES</div>
                {TEXT_PASSAGE_SCHEMA.properties.map(p=>(
                  <div key={p.name} style={{marginBottom:6,borderLeft:`2px solid ${C.crimson}`,paddingLeft:7}}>
                    <div style={{display:"flex",gap:6,alignItems:"baseline"}}>
                      <Mono col={C.crimson}>{p.name}</Mono>
                      <Mono col={C.slate} s={8}>{p.type}</Mono>
                    </div>
                    <div style={{fontSize:8,color:C.dim,marginTop:1}}>{p.note}</div>
                  </div>
                ))}
              </div>
              <div>
                <div style={{fontSize:8,color:C.dim,fontWeight:"bold",marginBottom:5}}>RELATIONSHIPS</div>
                {TEXT_PASSAGE_SCHEMA.relationships.map(r=>(
                  <div key={r.type} style={{marginBottom:6,borderLeft:`2px solid ${C.navy}`,paddingLeft:7}}>
                    <div style={{display:"flex",gap:6,alignItems:"baseline",flexWrap:"wrap"}}>
                      <Mono col={C.crimson}>{r.type}</Mono>
                      <span style={{fontSize:8,color:C.dim}}>→</span>
                      <Mono col={C.navy} s={8}>{r.target}</Mono>
                    </div>
                    <div style={{fontSize:8,color:C.dim,marginTop:1}}>{r.note}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{border:`1px solid ${C.rule}`,borderRadius:8,padding:12,marginBottom:12,background:"white"}}>
            <div style={{fontWeight:"bold",fontSize:10,color:C.ink,marginBottom:8}}>
              Entity Resolution Chain
            </div>
            <div style={{display:"flex",gap:0,alignItems:"center",flexWrap:"wrap",fontSize:9}}>
              {[
                ["IA searchinside", C.crimson, "query term"],
                ["→ hit: page + text", C.slate, ""],
                ["→ :TextPassage", C.crimson, "passage_id"],
                ["→ APPEARS_IN", C.navy, ""],
                ["→ :BibliographySource", C.navy, "ia_identifier"],
                ["→ NER pass", C.purple, ""],
                ["→ MENTIONS", C.navy, ""],
                ["→ :Entity", C.teal, "qid"],
              ].map(([label,col,sub],i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:3,marginBottom:4}}>
                  <span style={{background:col+"15",border:`1px solid ${col}`,borderRadius:4,
                    padding:"2px 6px",color:col,fontWeight:"bold",fontSize:8}}>{label}</span>
                  {sub&&<span style={{fontSize:7,color:C.dim}}>({sub})</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontSize:8,color:C.dim,fontWeight:"bold"}}>CREATE CYPHER</span>
              <CopyBtn text={TEXT_PASSAGE_SCHEMA.cypher_create}/>
            </div>
            <pre style={{fontFamily:"'Courier New',monospace",fontSize:8.5,color:C.teal,
              background:C.teal+"08",borderRadius:4,padding:8,margin:0,overflow:"auto"}}>{TEXT_PASSAGE_SCHEMA.cypher_create}</pre>
          </div>
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontSize:8,color:C.dim,fontWeight:"bold"}}>QUERY — passages mentioning an entity</span>
              <CopyBtn text={TEXT_PASSAGE_SCHEMA.cypher_query}/>
            </div>
            <pre style={{fontFamily:"'Courier New',monospace",fontSize:8.5,color:C.teal,
              background:C.teal+"08",borderRadius:4,padding:8,margin:0,overflow:"auto"}}>{TEXT_PASSAGE_SCHEMA.cypher_query}</pre>
          </div>
        </div>
      )}
      ...
      {section==="seed" && (
        <div>
          <div style={{fontSize:9,color:C.slate,marginBottom:10,background:"white",
            ...
            {IA_SEED_ITEMS.length} confirmed IA items ready for harvest.{" "}
            <strong style={{color:C.crimson}}>ia_identifier</strong> = new property on :BibliographySource.{" "}
            <strong style={{color:C.navy}}>gutenberg_id</strong> = property for Gutenberg-only items (no IA scan).
          </div>
          {IA_SEED_ITEMS.map((item,i)=>(
            <div key={i} style={{border:`1px solid ${C.rule}`,borderRadius:5,marginBottom:6,
              borderLeft:`3px solid ${item.ia_identifier?C.crimson:C.navy}`}}>
              <div style={{padding:"7px 10px",background:"white",display:"flex",gap:8,alignItems:"flex-start",flexWrap:"wrap"}}>
                <div style={{flex:"0 0 auto"}}>
                  {item.ia_identifier
                    ? <span style={{background:C.crimson+"15",border:`1px solid ${C.crimson}`,borderRadius:4,
                        padding:"1px 7px",fontSize:7.5,color:C.crimson,fontWeight:"bold"}}>IA SCAN</span>
                    : <span style={{background:C.navy+"15",border:`1px solid ${C.navy}`,borderRadius:4,
                        padding:"1px 7px",fontSize:7.5,color:C.navy,fontWeight:"bold"}}>GUTENBERG</span>
                  }
                </div>
                <div style={{flex:1}}>
                  <div style={{fontWeight:"bold",fontSize:10,color:C.ink}}>{item.title}</div>
                  <div style={{fontSize:9,color:C.slate,marginTop:1}}>{item.author}</div>
                  {item.ia_identifier&&(
                    <div style={{marginTop:4,display:"flex",gap:4,alignItems:"center"}}>
                      <Mono col={C.crimson} s={8}>{item.ia_identifier}</Mono>
                      <CopyBtn text={`https://api.archivelab.org/books/${item.ia_identifier}/searchinside?q=`}/>
                      <span style={{fontSize:7.5,color:C.dim}}>searchinside endpoint</span>
                    </div>
                  )}
                  {item.gutenberg_id&&(
                    <div style={{marginTop:4,display:"flex",gap:4,alignItems:"center"}}>
                      <Mono col={C.navy} s={8}>#{item.gutenberg_id}</Mono>
                      <CopyBtn text={`https://www.gutenberg.org/ebooks/${item.gutenberg_id}.txt.utf-8`}/>
                      <span style={{fontSize:7.5,color:C.dim}}>plain text URL</span>
                    </div>
                  )}
                  {item.notes&&<div style={{fontSize:8,color:C.dim,marginTop:4,fontStyle:"italic"}}>{item.notes}</div>}
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:2,flex:"0 0 auto"}}>
                  {item.facets.map(f=><Tag key={f} label={f} fill={FACET_COLOR[f]||C.slate}/>)}
                </div>
              </div>
            </div>
          ))}
      ...
      {section==="harvest" && (
        <div>
          <div style={{fontWeight:"bold",fontSize:11,color:C.ink,marginBottom:8}}>
            Harvest Pipeline: IA_SEARCHINSIDE → :TextPassage → MENTIONS :Entity
          </div>

          {/* Pipeline diagram */}
          <div style={{background:"white",border:`1px solid ${C.rule}`,borderRadius:8,
            padding:12,marginBottom:12}}>
            {[
              {step:"1",label:"Seed BibliographySource nodes",col:C.navy,
                desc:"One :BibliographySource per IA_SEED_ITEMS entry. Set ia_identifier or gutenberg_id property."},
              {step:"2",label:"Resolve all :Entity nodes needing text evidence",col:C.teal,
                desc:"MATCH (e:Entity) WHERE e.entity_type IN ['PERSON','PLACE','EVENT'] AND NOT exists(e.text_harvested)"},
              {step:"3",label:"For each (entity, source) pair → searchinside query",col:C.crimson,
                desc:"GET api.archivelab.org/books/{ia_id}/searchinside?q={entity.label} — returns page + text + bbox"},
              {step:"4",label:"Upsert :TextPassage nodes",col:C.crimson,
                desc:"MERGE on passage_id (SHA256 of ia_id+page+text[:40]). Set text, page_num, source_type='searchinside'."},
              {step:"5",label:"Wire APPEARS_IN and MENTIONS edges",col:C.purple,
                desc:"MERGE (tp)-[:APPEARS_IN {page_num}]->(bs). MERGE (tp)-[:MENTIONS {confidence}]->(entity)."},
              {step:"6",label:"NER pass on djvu.txt full text (background batch)",col:C.amber,
                desc:"Download djvu.txt → chunk by paragraph → NER → MERGE additional :TextPassage + MENTIONS edges. Complements searchinside hits."},
              {step:"7",label:"IA_PAGES on demand",col:C.rose,
                desc:"When UI needs to show a passage, resolve page image: GET api.archivelab.org/books/{ia_id}/pages/{page_num}"},
            ].map(({step,label,col,desc})=>(
              <div key={step} style={{display:"flex",gap:10,marginBottom:8,alignItems:"flex-start"}}>
                <div style={{background:col,color:"white",borderRadius:"50%",width:20,height:20,
                  display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:9,fontWeight:"bold",flexShrink:0,marginTop:1}}>{step}</div>
                <div>
                  <div style={{fontWeight:"bold",fontSize:9.5,color:col}}>{label}</div>
                  <div style={{fontSize:8.5,color:C.slate}}>{desc}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontSize:8,color:C.dim,fontWeight:"bold"}}>HARVEST PSEUDOCODE (agent loop)</span>
              <CopyBtn text={TEXT_PASSAGE_SCHEMA.harvest_pseudocode}/>
            </div>
            <pre style={{fontFamily:"'Courier New',monospace",fontSize:8.5,color:C.crimson,
              background:C.crimson+"06",borderRadius:4,padding:8,margin:0,
              overflow:"auto",maxHeight:280,whiteSpace:"pre-wrap"}}>{TEXT_PASSAGE_SCHEMA.harvest_pseudocode}</pre>
          </div>

          {/* sin=TXT cross-corpus search */}
          <div style={{background:C.amber+"0A",border:`1.5px solid ${C.amber}`,borderRadius:6,padding:10}}>
            <div style={{fontWeight:"bold",color:C.amber,fontSize:9.5,marginBottom:4}}>
              Cross-corpus full-text search (sin=TXT)
            </div>
            <div style={{fontSize:8.5,color:C.slate,marginBottom:6}}>
              To find which IA items mention a term across the entire Gutenberg/universallibrary corpus simultaneously — before assigning ia_identifiers to individual sources:
            </div>
            <div style={{fontFamily:"monospace",fontSize:8,color:C.amber,
              background:C.ink,borderRadius:4,padding:8,wordBreak:"break-all"}}>
              https://archive.org/advancedsearch.php?q="Tiberius+Gracchus"+AND+mediatype:texts&sin=TXT&fl[]=identifier,title,creator&output=json&rows=50
            </div>
            <div style={{fontSize:8,color:C.dim,marginTop:4}}>
              sin=TXT shifts search from item metadata to the OCR text index. Returns items where the phrase appears in the scanned text body — including back-of-book indexes and footnotes.
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"Georgia,serif",background:C.paper,minHeight:"100vh",padding:16}}>
      ...
      {view==="textpassage"  && <TextPassageView/>}
    </div>
  );
}
