...
// sc_rr_early_republic  : -509 to -264
// sc_rr_middle_republic : -264 to -133
// sc_rr_late_republic   : -133 to -27
// A person can belong to multiple periods (career spans a boundary).
MATCH (sc:SubjectConcept {subject_id: 'sc_rr_early_republic'})
MATCH (p:Person)
...
MATCH (sc:SubjectConcept {subject_id: 'sc_rr_middle_republic'})
MATCH (p:Person)
WHERE p.dprr_id IS NOT NULL
  AND p.floruit_start IS NOT NULL
...
MATCH (sc:SubjectConcept {subject_id: 'sc_rr_late_republic'})
MATCH (p:Person)
WHERE p.dprr_id IS NOT NULL
  AND p.floruit_start IS NOT NULL
...
MATCH (sc:SubjectConcept)
WHERE sc.subject_id IN ['sc_rr_early_republic','sc_rr_middle_republic','sc_rr_late_republic']
OPTIONAL MATCH (sc)<-[r:MEMBER_OF]-()
WITH sc, count(r) AS n
